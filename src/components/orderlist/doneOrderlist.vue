  ...
  <order-header></order-header> 
    <div class="Ninfo"  v-if="Ninfo" style="background:#efeff0">
     ...
     <div  v-if="!Ninfo" class="content-list"
      ...
      style=""
      >
        <div class="xin-li" v-for="(order,index) in doneOrderlist"  name='div'>
            <div class="li-one">
            ...
            <div class="goods">
                ...
                <div class="goods-button">
                  ...
                  <div >
                    <span class="ci"   @click="buyAgain(order)" >再次购买</span>
                    </div>
...
export default {
  ...
  created() {
    ...
    this.$http.get(this.apiUrl, {
      ...
      .then((data) => {
        this.doneOrderlist = data.data.data;
   ...
   buyAgain(order){
    if (order.goods.length == 1) {
        this.$router.push({
          name: 'prodetail',
          params: {
            prodetailId: order.goods[0].goods_id,
            goodsAttrId: order.goods[0].goods_attr_id,
            goodsNum: 1
          }
        })
      } else {
        ...
        for (var i = 0; i < order.goods.length; i++) {
          this.$http.get(this.addcarUrl, {
              params: {
                goodsId: order.goods[i].goods_id,
                goodsNum: order.goods[i].goods_sum,
                goodsAttrId: order.goods[i].goods_attr_id
              }
            })
            .then((res) => {
    ...
    loadMoreOrder: function() {
      ...
      this.$http.get(this.apiUrl, {
          params: {
           ...
           if (content) {
            this.doneOrderlist = this.doneOrderlist.concat(content)
         }
...
.content-list{
  padding-left:0.3125rem;padding-right:0.3125rem;background:#efeff0;overflow-y:scroll;position:absolute; top:3.25rem; height:81%;width:10rem;padding-bottom:1rem;
