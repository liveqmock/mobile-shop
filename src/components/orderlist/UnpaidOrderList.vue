 <template>
    ...
    <div class="Ninfo" v-if="Ninfo"   style="background:#efeff0">
      ...
      >
        <div class="xin-li" v-for="(order,index) in UnpaidOrderList"  name='div'>
            <div class="li-one">
            ...
            <div class="goods">
                ...
                <div class="goods-button">
                  ...
                  <div >
                    <span class="zhu"  @click="cancelOrder(order,index)" >取消订单</span>
                    <span class="ci" @click="isPay(order)" >去支付</span>
                  </div>
...
<script >
 import {MessageBox,InfiniteScrol,Toast} from 'mint-ui';
...
export default {
  ...
  created() {
    ...
    this.$http.get(this.apiUrl, {
      ...
      .then((data) => {
         this.UnpaidOrderList = data.data.data;
 ...
 // 获取订单的数量
  ...
  // 取消订单
    cancelOrder: function(order, index) {
        var order = order;
        var index =index;
     
       MessageBox.confirm('是否取消订单?').then(action => {
          this.$http.get('index/cancelOrder', {
            params: {
            ...
            if (data.body.error == 0) {
              ...
              this.UnpaidOrderList.splice(index, 1);
              this.getOrderNum()
            }
          })
        }).catch(function(err) {
    ...
    isPay (a) {
         window.location.href = 'poor/pay/orderSn/' + a.order_id
      },
        ...
        this.$http.get(this.apiUrl, {
          ...
          if (data.data.error == 99) {
            ...
            let content = data.data.data;
             this.UnpaidOrderList = this.UnpaidOrderList.concat(content)
          };
